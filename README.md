...
See [example/helmfile.yaml](./example/helmfile.yaml) for a helmfile-based example that installs both `kubernetes` and `node-exporter` mixins alongside `kube-prometheus-stack`.

## Values

For available values, see the [chart README](./chart/README.md).
